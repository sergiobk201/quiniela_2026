...
- [x] **`third_place_qualifier_ids` admin input** — ✅ SHIPPED 2026-06-16 (Day 19), commit `6ab6198`. The derive-from-standings fallback was actively harmful, not just inaccurate: with the column always null (no admin UI ever wrote it), the engine derived "3rd place per group" from **partial** standings and, the moment 8 groups had any finished match, scored everyone's qualifier picks against a meaningless mid-tournament table — inflating `pre_tournament_points` (Sergio: 6 → 15 with no new picks). Fix shipped: (1) `compute-scores` removes the fallback entirely — qualifiers score ONLY when `third_place_qualifier_ids` holds exactly 8 admin-entered teams; (2) `saveTournamentResults` persists the column via `parseQualifiers()`, returning the array only when all 8 are distinct (else null, so a partial entry can't trip the engine); (3) `/admin/scoring` gains an 8-team selector. No migration (column already existed). Requires `supabase functions deploy compute-scores` + recompute to clear stale phantom points.
